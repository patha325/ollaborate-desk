...
Only the local process connects to Ollama on the loopback interface. The source folder is read during indexing. The task runner receives snippets, paths, and LibreIndex's grounded answer. Ollaborate's planner and writer then produce a draft. The user decides whether to save it. The index, task history, and saved drafts live under `OLL_DESK_DATA`.
